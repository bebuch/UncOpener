...
UncOpener enables seamless opening of UNC network paths directly from your web browser. When installed and registered, it intercepts URLs with a custom scheme (default: `uncopener://server/share/path`) generated by the UncClickable extension and opens the corresponding network location.
...
- **Windows**: Opens the UNC path directly (e.g., `\\server\share\path`)
- **Linux**: Converts to SMB URL (e.g., `smb://server/share/path`) for file manager integration
